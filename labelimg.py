...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def add_zoom(self, increment=10):
        new_value = self.zoom_widget.value() + increment
        self.set_zoom(int(new_value))


    def zoom_request(self, delta):
        # Получаем текущие позиции полос прокрутки
        # Вычисляем процентные соотношения ~ координаты
        h_bar = self.scroll_bars[Qt.Horizontal]
        v_bar = self.scroll_bars[Qt.Vertical]

        # Получаем текущие максимальные значения для расчета разницы после масштабирования
        h_bar_max = h_bar.maximum()
        v_bar_max = v_bar.maximum()

        # Получаем позицию курсора и размер холста
        # Вычисляем желаемое перемещение от 0 до 1
        # где 0 = движение влево, 1 = движение вправо
        ...
        # Добавляем небольшой отступ, чтобы не нужно было попадать точно в самый левый или правый край
        margin = 0.1
        ...
        units = delta / (8 * 15)
        scale = 10
        self.add_zoom(int(scale * units))  # Преобразуем значение к int

        # Получаем разницу в значениях полос прокрутки
        # Это показывает, насколько мы можем двигаться
        d_h_bar_max = h_bar.maximum() - h_bar_max
        d_v_bar_max = v_bar.maximum() - v_bar_max

        # Получаем новые значения для полос прокрутки
        new_h_bar_value = h_bar.value() + move_x * d_h_bar_max
        new_v_bar_value = v_bar.value() + move_y * d_v_bar_max

        # Устанавливаем новые значения для полос прокрутки
        h_bar.setValue(int(new_h_bar_value))  # Преобразуем значение к int
        v_bar.setValue(int(new_v_bar_value))  # Преобразуем значение к int
